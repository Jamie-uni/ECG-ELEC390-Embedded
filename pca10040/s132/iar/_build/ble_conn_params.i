...
static _Bool is_conn_params_ok(ble_gap_conn_params_t const * p_preferred_conn_params,
...
{
    if (m_conn_params_config.error_handler != 0)
    {
...
static _Bool send_update_request(uint16_t conn_handle, ble_gap_conn_params_t * p_new_conn_params)
{
    ret_code_t err_code;
...
static void update_timeout_handler(void * p_context)
{
    ...
    if (p_instance != 0)
    {
        ...
        {
            _Bool update_sent = send_update_request(conn_handle, &p_instance->preferred_conn_params);
            if (update_sent)
            {
...
ret_code_t ble_conn_params_init(const ble_conn_params_init_t * p_init)
{
    ...
    do { if ((((p_init) == 0))) { return ((0x0) + 14); } } while (0);
...
static_assert(1, "NRF_SDH_BLE_ENABLED not set!"); static_assert(1 < 4, "Priority level unavailable."); __root static nrf_sdh_ble_evt_observer_t m_ble_observer @ "sdh_ble_observers1" = { . handler = ble_evt_handler, . p_context = 0 };
