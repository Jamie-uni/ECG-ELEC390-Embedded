...
extern nrf_log_module_dynamic_data_t m_nrf_log_ble_gatt_logs_data_dynamic;
...
static_assert(247 <= 251, "unspecified message");
static_assert(247 + 4 <= 255, "unspecified message");
...
static void link_init(nrf_ble_gatt_link_t * p_link)
{
    ...
    p_link->att_mtu_exchange_requested = 0;

    p_link->data_length_desired        = 247 + 4;
    p_link->data_length_effective      = 23 + 4;
...
static void on_connected_evt(nrf_ble_gatt_t * p_gatt, ble_evt_t const * p_ble_evt)
{
    ...
    if (p_link->att_mtu_desired > p_link->att_mtu_effective)
    {
        ...
        if (err_code == ((0x0) + 0))
        {
            p_link->att_mtu_exchange_requested = 1;
        }
...
static void on_exchange_mtu_rsp_evt(nrf_ble_gatt_t * p_gatt, ble_evt_t const * p_ble_evt)
{
    ...
    if ((p_gatt->evt_handler != 0) && (p_link->att_mtu_exchange_requested))
    {
                                        ...
                                        uint8_t          data_length)
{
    ...
    {
        p_gatt->data_length = ((data_length) < (247 + 4) ? (data_length) : (247 + 4));
        return ((0x0) + 0);
    }
    ...
    err_code = sd_ble_gap_data_length_update(conn_handle, &dlp, 0);
    return err_code;
}
