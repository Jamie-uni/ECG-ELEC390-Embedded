...
#line 46 "C:\\nRF5_SDK_14.2.0_17b948a\\components\\ble\\ble_advertising\\ble_advertising.c"
#line 1 "C:\\nRF5_SDK_14.2.0_17b948a\\examples\\ble_peripheral\\ble_app_hrs\\pca10040\\s132\\iar\\..\\..\\..\\..\\..\\..\\components\\libraries\\fstorage\\nrf_fstorage.h"
...
#line 62 "C:\\nRF5_SDK_14.2.0_17b948a\\components\\ble\\ble_advertising\\ble_advertising.c"
    ...
    static _Bool whitelist_has_entries(ble_advertising_t * const p_advertising)
    {
...
static void on_disconnected(ble_advertising_t * const p_advertising, ble_evt_t const * p_ble_evt)
{
    ...
    p_advertising->whitelist_temporarily_disabled = 0;

    if (p_ble_evt->evt.gap_evt.conn_handle == p_advertising->current_slave_link_conn_handle &&
        p_advertising->adv_modes_config.ble_adv_on_disconnect_disabled == 0)
    {
       ret = ble_advertising_start(p_advertising, BLE_ADV_MODE_DIRECTED);
       if ((ret != ((0x0) + 0)) && (p_advertising->error_handler != 0))
       {
...
{
    _Bool peer_addr_is_valid = addr_is_valid(p_advertising->peer_address.addr);
                                    ...
                                    ble_gap_adv_params_t    * p_adv_params)
{
        ...
        (whitelist_has_entries(p_advertising)))
    {
        ...
        p_advertising->advdata.flags  = (0x04);

        ret = ble_advdata_set(&(p_advertising->advdata), 0);
        if (ret != ((0x0) + 0))
        {
            return ret;
        }
...
{
    uint32_t ret;
    if ((p_init == 0) || (p_advertising == 0))
    {
        return ((0x0) + 14);
    }

    p_advertising->initialized                    = 1;
    ...
    if (p_init->advdata.p_manuf_specific_data != 0)
    {
    ...
    if (p_init->advdata.p_service_data_array != 0)
    {
    ...
    if (p_init->advdata.p_slave_conn_int != 0)
    {
    ...
    if (p_init->advdata.p_tx_power_level != 0)
    {
                               ...
                               ble_adv_mode_t            advertising_mode)
{
    ...
    if (p_advertising->initialized == 0)
    {
        return ((0x0) + 8);
    }
    ...
    {
        p_advertising->advertising_start_pending = 1;
        return ((0x0) + 0);
    }
   ...
   memset(&p_advertising->peer_address, 0, sizeof(p_advertising->peer_address));
    ...
    {
        if (p_advertising->evt_handler != 0)
        {
            p_advertising->peer_addr_reply_expected = 1;
            p_advertising->evt_handler(BLE_ADV_EVT_PEER_ADDR_REQUEST);
        }
        else
        {
            p_advertising->peer_addr_reply_expected = 0;
        }
    }
        ...
        (!p_advertising->whitelist_temporarily_disabled))
    {
            ...
            p_advertising->whitelist_in_use = 0;

        p_advertising->whitelist_reply_expected = 1;
        p_advertising->evt_handler(BLE_ADV_EVT_WHITELIST_REQUEST);
    }
    else
    {
        p_advertising->whitelist_reply_expected = 0;
    }
...
void ble_advertising_on_sys_evt(uint32_t evt_id, void * p_context)
{
    ...
    switch (evt_id)
    {
        ...
        case NRF_EVT_FLASH_OPERATION_ERROR:
        {
            ...
            {
                p_advertising->advertising_start_pending = 0;
...
uint32_t ble_advertising_restart_without_whitelist(ble_advertising_t * const p_advertising)
{
    ...
    p_advertising->whitelist_temporarily_disabled = 1;


        p_advertising->whitelist_in_use = 0;


    p_advertising->advdata.flags = ((0x02) | (0x04));
