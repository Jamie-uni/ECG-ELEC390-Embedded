...
__root nrf_fstorage_t m_fs @ "fs_data" =
{
...
static fds_op_t m_queue_data[(4)+1]; static nrf_atfifo_t m_queue_inst; static nrf_atfifo_t * const m_queue = &m_queue_inst;
...
static _Bool header_has_next(fds_header_t const * p_hdr, uint32_t const * p_page_end)
{
...
static ret_code_t page_tag_write_swap(void)
{
    ...
    return nrf_fstorage_write(&m_fs, (uint32_t)m_swap_page.p_addr, page_tag_swap, (2) * sizeof(uint32_t), 0);
}
...
static ret_code_t page_tag_write_data(uint32_t const * const p_page_addr)
{
    ...
    return nrf_fstorage_write(&m_fs, (uint32_t)p_page_addr, page_tag_data, (2) * sizeof(uint32_t), 0);
}
...
static _Bool queue_has_next(void)
{
    
    if (0) { if (m_queued_op_cnt != 0) { } else { assert_nrf_callback((uint16_t)625, (uint8_t *)"C:\\nRF5_SDK_14.2.0_17b948a\\components\\libraries\\fds\\fds.c"); } };
...
static ret_code_t record_header_write_begin(fds_op_t * const p_op, uint32_t * const p_addr)
{
    ...
    ret = nrf_fstorage_write(&m_fs, (uint32_t)(p_addr + (0)),
        &p_op->write.header.record_key, (1) * sizeof(uint32_t), 0);
...
static ret_code_t record_header_write_finalize(fds_op_t * const p_op, uint32_t * const p_addr)
{
    ...
    ret = nrf_fstorage_write(&m_fs, (uint32_t)(p_addr + (1)),
        &p_op->write.header.file_id, (1) * sizeof(uint32_t), 0);
...
static ret_code_t record_header_flag_dirty(uint32_t * const p_record, uint16_t page_to_gc)
{
    ...
    _Pragma("data_alignment = 4") static uint32_t const dirty_header = {0xFFFF0000};

    
    ret_code_t ret;

    ret = nrf_fstorage_write(&m_fs, (uint32_t)p_record,
        &dirty_header, (1) * sizeof(uint32_t), 0);
...
static ret_code_t record_write_data(fds_op_t * const p_op, uint32_t * const p_addr)
{
    ...
    ret = nrf_fstorage_write(&m_fs, (uint32_t)(p_addr + (3)),
        p_op->write.p_data,  p_op->write.header.length_words * sizeof(uint32_t), 0);
...
static ret_code_t gc_swap_erase(void)
{
    ...
    return nrf_fstorage_erase(&m_fs, (uint32_t)m_swap_page.p_addr, (1024 / (1024)), 0);
}
...
static ret_code_t gc_page_erase(void)
{
    ...
    if (m_pages[gc].records_open == 0)
    {
        ...
        ret = nrf_fstorage_erase(&m_fs, (uint32_t)m_pages[gc].p_addr, (1024 / (1024)), 0);
    }
    else
    {
...
static ret_code_t init_execute(uint32_t prev_ret, fds_op_t * const p_op)
{
    ...
    switch (p_op->init.step)
    {
        ...
        case FDS_OP_INIT_ERASE_SWAP:
        {
            ...
            ret = nrf_fstorage_erase(&m_fs, (uint32_t)m_swap_page.p_addr, (1024 / (1024)), 0);
        } break;
...
static void queue_process(ret_code_t result)
{
    ...
    while (1)
    {
        ...
        if (0) { if (m_p_cur_op != 0) { } else { assert_nrf_callback((uint16_t)1401, (uint8_t *)"C:\\nRF5_SDK_14.2.0_17b948a\\components\\libraries\\fds\\fds.c"); } };
...
static ret_code_t flash_subsystem_init(void)
{
    flash_bounds_set();
        ...
        return nrf_fstorage_init(&m_fs, &nrf_fstorage_sd, 0);
