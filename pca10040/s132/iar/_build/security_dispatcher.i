...
static_assert((0 == 0) || 0, "Peer Manager Central operation must be enabled when using central links.");
                           ...
                           pm_conn_sec_procedure_t procedure)
{
    ...
    if (success)
    {
        ...
        ble_conn_state_user_flag_set(conn_handle, m_flag_sec_proc_new_peer, 0);
                                  ...
                                  _Bool         new_peer_created)
{
    if (success)
    {
        if (new_peer_created)
        {
            ble_conn_state_user_flag_set(conn_handle, m_flag_sec_proc_new_peer, 1);
...
static void sec_info_request_process(ble_gap_evt_t const * p_gap_evt)
{
    ...
    sec_proc_start(p_gap_evt->conn_handle, 1, PM_LINK_SECURED_PROCEDURE_ENCRYPTION);
    ...
    if (err_code != ((0x0) + 0))
    {
        sec_proc_housekeeping(p_gap_evt->conn_handle, peer_id, 0, 0);
...
static void conn_sec_update_process(ble_gap_evt_t const * p_gap_evt)
{
    if (!pairing(p_gap_evt->conn_handle))
    {
        ...
        else
        {
            ...
            evt.params.conn_sec_succeeded.data_stored = 0;
                            ...
                            ble_gap_lesc_p256_pk_t * p_public_key)
{
    ...
    _Bool                 new_peer_created = peer_created(conn_handle);
    ...
    if (role == 0x1)
    {
        ...
        ble_conn_state_user_flag_set(conn_handle, m_flag_allow_repairing, 0);
    }
