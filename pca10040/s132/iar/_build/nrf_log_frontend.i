...
#line 47 "C:\\nRF5_SDK_14.2.0_17b948a\\components\\libraries\\experimental_log\\src\\nrf_log_frontend.c"
#line 1 "C:\\nRF5_SDK_14.2.0_17b948a\\examples\\ble_peripheral\\ble_app_hrs\\pca10040\\s132\\iar\\..\\..\\..\\..\\..\\..\\components\\libraries\\atomic\\nrf_atomic.h"
...
#line 1 "C:\\nRF5_SDK_14.2.0_17b948a\\components\\libraries\\atomic\\nrf_atomic_internal.h"
...
static_assert((1024 % 4) == 0, "unspecified message");
static_assert(( ((1024) != 0) && ((((1024) - 1) & (1024)) == 0) ), "unspecified message");
...
static_assert(((8)) <= 0xff, "unspecified message"); static uint8_t mempool_nrf_balloc_pool_stack[((8))]; static uint32_t mempool_nrf_balloc_pool_mem [(((((20)+sizeof(uint32_t))) - 1) + sizeof(uint32_t) - (((((20)+sizeof(uint32_t))) - 1) % sizeof(uint32_t))) * ((8)) / sizeof(uint32_t)]; static nrf_balloc_cb_t mempool_nrf_balloc_cb; static const nrf_balloc_t mempool = { . p_cb = &mempool_nrf_balloc_cb, . p_stack_base = mempool_nrf_balloc_pool_stack, . p_stack_limit = mempool_nrf_balloc_pool_stack + ((8)), . p_memory_begin = mempool_nrf_balloc_pool_mem, . block_size = (((((20)+sizeof(uint32_t))) - 1) + sizeof(uint32_t) - (((((20)+sizeof(uint32_t))) - 1) % sizeof(uint32_t))),   };
...
_Pragma("section = \"log_dynamic_data\"");;
_Pragma("section = \"log_const_data\"");;
 
__root const nrf_log_module_const_data_t m_nrf_log_app_logs_data_const @ "log_const_data" = { . p_module_name = "app", . info_color_id = 0, . debug_color_id = 0, . compiled_lvl = 3, }; __root nrf_log_module_dynamic_data_t m_nrf_log_app_logs_data_dynamic @ "log_dynamic_data";
...
ret_code_t nrf_log_init(nrf_log_timestamp_func_t timestamp_func)
{
    ...
    uint32_t modules_cnt = ((size_t)__section_end("log_const_data") - (size_t)__section_begin("log_const_data")) / sizeof(nrf_log_module_const_data_t);
    uint32_t i;
    if (0)
    {
        uint32_t j;
        ...
        for (i = 0; i < modules_cnt; i++)
        {
            ...
            for (j = 0; j < modules_cnt; j++)
            {
                if  (i != j)
                {
                    char const * p_name0 = ((nrf_log_module_const_data_t*)__section_begin("log_const_data") + ((i)))->p_module_name;
                    char const * p_name1 = ((nrf_log_module_const_data_t*)__section_begin("log_const_data") + ((j)))->p_module_name;
            ...
            nrf_log_module_dynamic_data_t * p_module_ddata = ((nrf_log_module_dynamic_data_t*)__section_begin("log_dynamic_data") + ((i)));
...
uint32_t nrf_log_module_cnt_get(void)
{
    return ((size_t)__section_end("log_const_data") - (size_t)__section_begin("log_const_data")) / sizeof(nrf_log_module_const_data_t);
}

static ret_code_t module_idx_get(uint32_t * p_idx, _Bool ordered_idx)
{
    if (ordered_idx)
    {
        ...
        {
            nrf_log_module_dynamic_data_t * p_module_data = ((nrf_log_module_dynamic_data_t*)__section_begin("log_dynamic_data") + ((i)));
...
const char * nrf_log_module_name_get(uint32_t module_id, _Bool ordered_idx)
{
    if (module_idx_get(&module_id, ordered_idx) == ((0x0) + 0))
    {
        nrf_log_module_const_data_t * p_module_data = ((nrf_log_module_const_data_t*)__section_begin("log_const_data") + ((module_id)));
...
void nrf_log_module_filter_set(uint32_t backend_id, uint32_t module_id, nrf_log_severity_t severity)
{
    if (0)
    {
        nrf_log_module_dynamic_data_t * p_module_filter = ((nrf_log_module_dynamic_data_t*)__section_begin("log_dynamic_data") + ((module_id)));
    ...
    if (0 && dynamic)
    {
        if (module_idx_get(&module_id, ordered_idx) == ((0x0) + 0))
        {
            nrf_log_module_dynamic_data_t * p_module_filter =
                                                ((nrf_log_module_dynamic_data_t*)__section_begin("log_dynamic_data") + ((module_id)));
    ...
    else if (!dynamic)
    {
        if (module_idx_get(&module_id, ordered_idx) == ((0x0) + 0))
        {
            nrf_log_module_const_data_t * p_module_data =
                                                ((nrf_log_module_const_data_t*)__section_begin("log_const_data") + ((module_id)));
...
static void log_skip(void)
{
    ...
    while (p_header->base.generic.type == 0U)
    {
        rd_idx       += (sizeof(nrf_log_pushed_header_t)/sizeof(uint32_t));
    ...
    switch (header.base.generic.type)
    {
        ...
        case 1U:
            ...
            break;
        default:
            if (0) { if (0) { } else { assert_nrf_callback((uint16_t)333, (uint8_t *)"C:\\nRF5_SDK_14.2.0_17b948a\\components\\libraries\\experimental_log\\src\\nrf_log_frontend.c"); } };
            break;
    }
                                      ...
                                      uint32_t mask)
{
    ...
    uint16_t module_id = severity_mid >> 16;
    if (0) { if (module_id < nrf_log_module_cnt_get()) { } else { assert_nrf_callback((uint16_t)356, (uint8_t *)"C:\\nRF5_SDK_14.2.0_17b948a\\components\\libraries\\experimental_log\\src\\nrf_log_frontend.c"); } };
    ...
    p_header->base.std.raw      = (severity_mid & (1UL << 4U)) ? 1 : 0;
...
static inline _Bool buf_prealloc(uint32_t content_len, uint32_t * p_wr_idx)
{
    uint32_t req_len = content_len + (sizeof(nrf_log_header_t)/sizeof(uint32_t) - (0 ? 0 : 1));
    uint32_t ovflw_tag_size = (sizeof(nrf_log_header_t)/sizeof(uint32_t) - (0 ? 0 : 1));
    _Bool     ret            = 1;
    ...
    *p_wr_idx = m_log_data.wr_idx;
    ...
    uint32_t required_words  = req_len + ovflw_tag_size; 
    while (required_words > available_words)
    {
        ...
        {
            if (available_words >= (sizeof(nrf_log_header_t)/sizeof(uint32_t) - (0 ? 0 : 1)))
            {
                
                std_header_set(2UL, m_overflow_info, 0, m_log_data.wr_idx, m_log_data.mask);
                req_len = (sizeof(nrf_log_header_t)/sizeof(uint32_t) - (0 ? 0 : 1));
            }
            else
            {
...
uint32_t nrf_log_push(char * const p_str)
{
    if ((m_log_data.autoflush) || (p_str == 0))
    {
    ...
    {
        nrf_log_header_t * p_header = (nrf_log_header_t *)&m_log_data.buffer[wr_idx & mask];
        (p_header)->base . pushed . type = 0U; (p_header)->base . pushed . offset = offset; (p_header)->base . pushed . len = buflen;
                              ...
                              uint16_t           length)
{
    ...
    if (buf_prealloc((((length) + (sizeof(uint32_t)) - 1) / (sizeof(uint32_t))), &wr_idx))
    {
        ...
        m_log_data.buffer[(header_wr_idx + 1) & mask] = severity_mid >> 16;
        
        nrf_log_header_t * p_header = (nrf_log_header_t *)&m_log_data.buffer[header_wr_idx & mask];
        p_header->base.hexdump.raw      = (severity_mid & (1UL << 4U)) ? 1 : 0;
...
_Bool nrf_log_frontend_dequeue(void)
{
    ...
    else if (header.base.generic.type == 3U && (m_log_data.log_skipped == 0))
    {
    ...
    if (p_msg_buf)
    {
        ...
        else
        {
            while (p_backend)
            {
                _Bool entry_accepted = 0;
                if (nrf_log_backend_is_enabled(p_backend) == 1)
                {
                    if (0)
                    {
                        uint8_t backend_id = nrf_log_backend_id_get(p_backend);
                        uint32_t filter_lvls = ((nrf_log_module_dynamic_data_t*)__section_begin("log_dynamic_data") + ((header . module_id)))->filter_lvls;
...
int32_t nrf_log_backend_add(nrf_log_backend_t * p_backend, nrf_log_severity_t severity)
{
    ...
    if (m_log_data.p_backend_head == 0)
    {
    ...
    if (0)
    {
        ...
        {
            nrf_log_severity_t buildin_lvl = nrf_log_module_filter_get(id, i, 0, 0);
